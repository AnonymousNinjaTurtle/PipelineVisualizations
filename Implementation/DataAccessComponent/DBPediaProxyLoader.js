...
export default class DBpediaProxyLoader extends SimpleProxyLoader {
  constructor() {
    super();
    ...
    this.suffix = '&format=application%2Fsparql-results%2Bjson&CXML_redir_for_subjs=121&CXML_redir_for_hrefs=&timeout=30000&debug=on&run=+Run+Query+';
    this.exampleQuery = 'select ?subject ?predicate ?object where { ?subject ?predicate ?object} Limit 10';
    this.teslaExample =
  ...
  _buildDataRequest = () => {
    ...
    const requestPath =
      this.dbPediaURL + '?default-graph-uri=' + encodeURIComponent(this.dbPediaGraph) + '&query=' + encodeURIComponent(this.query) + this.suffix;
